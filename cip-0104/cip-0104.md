...
Let `total_burn` be the amount of traffic bytes consumed by the confirmation request.
At a high-level, the app activity record computation works as follows:
...
3. If it is featured-app submitted, then
   1. attribute the `total_burn` as activity to the submitting app and
   2. if the submitting app is a featured asset mover,
      then split `total_burn * extraFeaturedAssetActivityWeight` among the involved featured assets proportional to the sizes of the envelopes they confirm
      and apply special provisions so that:
        - no asset receives more than `total_burn * maxExtraFeaturedAssetActivityWeightPerAsset`
...
We explain the detailed computation steps below.
...
TODO: rename `per_app_traffic_weight` to `per_app_burn`

For user-submitted transactions, the `total_burn` is split among the featured app confirmers as follows:
...
1. Determine the app confirmers as the parties for which a `FeaturedAppRight` is active at round start time.
2. Determine `total_app_envelopes_traffic` as the total amount of traffic of envelopes that have at least one app confirmer.
4. For every envelope with `num_app_confirmers > 0`, compute

   ```
   per_app_traffic_weight =
    (envelope_traffic_cost * total_burn) / (total_app_envelopes_traffic * num_app_confirmers)
   ```

   using integer arithmetic, and attribute `per_app_traffic_weight` to every app confirmer of the envelope.

Note that the computation in Step 4 takes care of distributing the traffic cost not attributed to app envelopes in a weighted fashion among the app envelopes.
...
If the submitting app is a featured asset mover,
then `total_burn * extraFeaturedAssetActivityWeight` is split among the involved featured assets as follows:

1. Compute the confirmers of an envelope as the union of the confirmers of the views in that envelope.
2. Determine the asset confirmers as the parties which are either the `dso` party or which are marked as featured asset providers at round start time.
3. Determine `assetEnvelopeBurn[A]` for party `A` by summing up `envelopeBurn[E] / numAssetConfirmers[E]` for every envelope `E` with `A` as an asset confirmer.
4. Determine `totalAssetBurn` as the sum of `assetEnvelopeBurn[A]` for every asset confirmer `A`.
5. Determine `assetActivityUncapped[A]` for every asset confirmer `A` as `floor (total_burn * extraFeaturedAssetActivityWeight) * assetEnvelopeBurn[A] / totalAssetBurn`.
6. Determine `assetActivityCap` as `maxExtraFeaturedAssetActivityWeightPerAsset * total_burn`.
