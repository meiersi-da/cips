...
Adjust the featured app reward cap to allow minting at most 79 cent for $1 of burn:

- set `issuanceCurve.initialValue.featuredAppRewardCap` to `0.79`
...
- **app category**: general featured app, featured asset-moving app, or featured asset. Default: general featured app
...
The activity record computation is designed to satisfy the following design objectives:
...
This guidance calls out that it is important to distinguish between the following app categories:
...
1. **split user activity among app providers:** burn from user-submitted transactions is split among all involved featured app providers
2. **rebate app submissions without arbitrage:** between 90% and 99% of the burn of featured-app-submitted transactions must be assigned to the submitting featured app
3. **reward assets enabling asset movers:** record extra activity for featured assets used in transactions submitted by asset-moving featured apps
4. **fair treatment of CC:** ensure that asset moving apps have equal incentives for integrating CC vs integrating featured assets

These are the objectives implemented by the mechanics described in this CIP.
