...
Furthermore, the proposed app rewards computation is carefully designed to not only incentivize general innovation,
but also the [composition of high-quality apps with assets](#rationale).
...
### Adjust Featured App Reward Cap

Adjust the featured app reward cap to allow minting at most 79 cents for $1 of burn:

- set `issuanceCurve.initialValue.featuredAppRewardCap` in `AmuletConfig` to `0.79`

### Introduce Featured App Metadata

Extend featured app rights with three kinds of metadata relevant for the computation of app activity records:

- **app category**: one of **general featured app**, **featured asset-moving app**, or **featured asset** as explained in [Rationale](#rationale). Default: general featured app
- **app operations parties**: extra parties that are used by the app to submit transactions in its name. Default: empty list
- **app composition weight**: the weight of the app used when [splitting extra burn among featured assets](#splitting-extra-burn-among-assets). Default: `1.0`

The operations parties are required for decentralized applications (e.g. cBTC),
which decentralize the featured app party and use delegation to operator parties
to submit transactions on behalf of the app.
They may also be used by other apps that use multiple operations parties (e.g., for increased security).
The operations parties of different apps are expected to be disjoint.

The featured app metadata is managed via SV voting,
and the SV app supports the SV operators to avoid duplicate registrations of operations parties.

### Extend Amulet Configuration

Add the following parameters to the Amulet configuration:

* `extraFeaturedAssetsActivityWeight : Decimal` with default `0.2`
* `maxExtraActivityWeightPerAsset : Decimal` with default `0.1`
* `dsoAppActivityWeight : Decimal` with default `0.5`

The `extraFeaturedAssetsActivityWeight` configures the extra activity weight shared among featured
assets when they are used in transactions submitted by featured asset-moving apps.
The `maxExtraActivityWeightPerAsset` sets an upper limit on the extra activity weight that any single featured asset can receive.
The `dsoAppActivityWeight` configures the weight applied to the DSO party when [splitting extra burn among featured assets](#splitting-extra-burn-among-assets).
...
Let `totalBurn` be the amount of traffic bytes consumed by the confirmation request.
At the top level, the app activity record computation works as follows:

1. Determine whether the transaction was submitted by a user or a featured app.
2. If it is user-submitted, then split total burn among the featured app confirmers proportional to the sizes of the envelopes they confirm.
3. If it is featured-app submitted, then
   1. attribute the `totalBurn` as activity to the submitting app and
   2. if the submitting app is a featured asset mover,
      then split `totalBurn * extraFeaturedAssetActivityWeight` among the involved featured assets proportional to the sizes of the envelopes they confirm
      and apply special provisions so that:
        - no asset receives more than `totalBurn * maxExtraFeaturedAssetActivityWeightPerAsset`
        - the excess activity from capped assets is redistributed among the remaining assets in proportion to their uncapped activity
        - app composition weights are applied to the resulting activity records
        - Canton Coin is considered a featured asset whose activity is assigned to the submitting app

We explain the details of the computation steps in the three sections below.

##### Determining app-submitted transactions

A transaction submitted by the list of parties `submitters` is considered to be **submitted by the featured app `X`** if these three conditions and the tie-breaker criterion below are met:

1. `X` is a confirmer.
2. `X` is a featured app at the start time of the round associated to the transaction.
3. `X` or one of `X`'s operational parties is included in `submitters`.

Ties are broken by first selecting the app whose submitter appears earliest in the list of `submitters`
and then by selecting the featured app with the lexicographically smallest identifier.

##### Splitting Total User Burn

For user-submitted transactions, the `totalBurn` is split among the featured app confirmers as follows:
...
3. Determine `appEnvelopeBurn[A]` for every app confirmer `A` by summing up `envelopeBurn[E] / numAppConfirmers[E]` for every envelope `E` with `A` as an app confirmer.
4. Determine `totalAppBurn` as the sum of `appEnvelopeBurn[A]` for every app confirmer `A`.
5. Determine `appActivity[A]` for every app confirmer `A` as `(totalBurn * appEnvelopeBurn[A]) / totalAppBurn)` and store the resulting activity for each app confirmer.

Note that the computation in Step 5 takes care of distributing the traffic cost not attributed to app envelopes in a weighted fashion among the app envelopes.
...
The computation is performed using integer arithmetic for efficiency reasons.
...
##### Splitting Extra Burn Among Assets

Note that transactions submitted by a featured app always attribute `totalBurn` of app activity to the submitting featured app.
If the submitting app `X` has a `FeaturedAppRight` marking it as a featured asset mover at round start,
then an extra amount of `totalBurn * extraFeaturedAssetActivityWeight` of app activity
is split among the involved featured assets as follows:

1. Determine the asset confirmers as the parties which are either the `dso` party or
   whose `FeaturedAppRight` marks them as featured asset providers at round start time.
2. Determine `assetEnvelopeBurn[A]` for party `A` by summing up `envelopeBurn[E] / numAssetConfirmers[E]` for every envelope `E` with `A` as an asset confirmer.
3. Determine `totalAssetBurn` as the sum of `assetEnvelopeBurn[A]` for every asset confirmer `A`.
4. Determine `assetActivityUncapped[A]` for every asset confirmer `A` as `(floor(totalBurn * extraFeaturedAssetActivityWeight) * assetEnvelopeBurn[A]) / totalAssetBurn`.
5. Determine `assetActivityCap` as `floor(maxExtraFeaturedAssetActivityWeightPerAsset * totalBurn)`.
6. Determine `assetActivityCapped[A]` for every asset confirmer `A` as `min(assetActivityUncapped[A], assetActivityCap)`.
7. Determine `totalCappedActivity` as the sum of `assetActivityUncapped[A] - assetActivityCapped[A]` for every asset confirmer `A` and redistribute it
   among the assets with uncapped activity proportional to their uncapped activity:
   1. Determine `totalUncappedActivity` as the sum of `assetActivityUncapped[A]` for every asset confirmer `A` with `assetActivityUncapped[A] < assetActivityCap`.
   2. Determine `cappedActivityShare[A]` for every asset confirmer `A` with `assetActivityUncapped[A] < assetActivityCap` as `(totalCappedActivity * assetActivityUncapped[A]) / totalUncappedActivity`.
   3. Update `assetActivityCapped[A]` for every asset confirmer `A` with `assetActivityUncapped[A] < assetActivityCap` as
      `min(assetActivityUncapped[A] + cappedActivityShare[A], assetActivityCap)`.
8.  For every asset confirmer `A != dso`, record activity `floor(assetActivityCapped[A] * appRight[X].appCompositionWeight * appRight[A].appCompositionWeight)`.
9. Record activity `floor(assetActivityCapped[dso] * appRight[X].appCompositionWeight * amuletConfig.dsoAppActivityWeight)` for the
   featured asset-moving app `X`.

The computations are by default performed using Int64 arithmetic except for the multiplications with the `Decimal` configuration parameters.
These are performed using Daml's `Decimal` arithmetic.
`floor` is applied to avoid over-issuance due to rounding up.

The computations in Step 7 ensure that in a DvP of two assets, both asset providers always receive
equal shares of the `totalBurn` independently of the relative sizes of the envelopes affecting them.
For batch settlements involving multiple assets, they ensure that activity is distributed relative
to the amount of change to each assets' ownership register.
See the [calculation examples](#example-views-and-envelopes-for-dvp-settlement) for more details.
...
1. Compute the `totalFeaturedAppActivity` by summing up all activity record weights for the round.
2. Determine the `traffic_price_in_CC_per_MB` using the `AmuletRules` contract at round start time.
3. Compute `total_featured_app_burn_CC = (totalFeaturedAppActivity / 1e6) * traffic_price_in_CC_per_MB`.
...
6. Compute the `per_app_minting_allowance` for every party `A`, by computing the `totalAppActivity[A]` as the sum of the `A`'s activity record weights for the round, and setting
   ```
   appMintingAllowance0[A] =
     (totalAppActivity[A] / 1e6) * traffic_price_in_CC_per_MB *
     issuance_per_featured_app_weight

   appMintingAllowance[A] =
     if appMintingAllowance0[A] >= app_reward_coupon_threshold_CC
       then appMintingAllowance0[A]
       else 0
   ```
...
### Motivation for App Activity Computation

The app reward mechanics proposed in this CIP are based on the learnings of the Tokenomics committee
managing the marker-based app reward system on MainNet.
These learnings evolve with time.
At the time of writing, the most recent learnings are captured in the ["Featured App Guidance v2.0b"](https://docs.google.com/document/d/1G-FVUU_MllU4StgExg3oiODbvKD-LIgiojEmc__Bqt8/edit?tab=t.0#heading=h.22k6ymijl3pg) document,
which forms the basis for the mechanics proposed by this CIP.
We recommend reading that document to understand the rationale for the guidance.

The guidance distinguishes between the following app categories:

* **general featured app:** anyone can obtain this status simply by clearing the featured app locking requirements and not abusing the privileges associated with it.
* **asset-moving featured app:** a restricted set of featured apps that provide value by enabling users to move assets. Additional requirements for security, operational maturity, and standards compliance apply.
* **featured asset:** a restricted set of featured apps for asset providers. They must lock extra funds, and meet additional security, standards compliance, and operational requirements.

Based on these categories the guidance can be distilled into the following key design objectives:

1. **split user activity among app providers:** split burn from user-submitted transactions among all involved featured app providers.
2. **rebate app submissions without arbitrage:** assign between 90% and 99% of the burn of featured-app-submitted transactions to the submitting featured app.
3. **reward assets enabling asset movers:** attribute an extra 20% of burn to the featured assets used in transactions submitted by asset-moving featured apps, with at most 10% going to any single featured asset.
4. **governance-based fine tuning:** allow fine-tuning of the asset-movement incentives via SV governance.
5. **fair treatment of CC:** ensure that asset moving apps have equal incentives for integrating CC vs integrating featured assets.

The subsections below provide details on how the proposed app activity computation addresses the key design objectives outlined above.

#### Splitting User Activity
...
This is why the `totalBurn` is distributed among the featured apps, and not not only the burn for
envelopes with a featured app confirmer.


#### Avoiding Arbitrage

An app submitting a transaction costing $1 in burn receives at most `featuredAppRewardCap + validatorRewardCap` in rewards.
With the change to the `featuredAppRewardCap` to `0.79` proposed in this CIP, they can thus receive at most `0.79 + 0.2 = $0.99` in rewards,
which is just below the total burn of $1 to prevent arbitrage opportunities.

The objective above mentions a range from 90% to 99% to leave space for the adjustment of the validator rewards that is
being proposed in [CIP-0120](../cip-0120/cip-0120.md), which at the time of
writing would typically result in >= 95% of the burn being assigned to the
submitting featured app.

#### Rewarding Assets Enabling Asset Movers

Asset moving apps could not exist without high-quality assets that users want to trade.
To incentivize the creation and maintenance of such high-quality assets, the protocol attributes an extra portion of the burn to the featured assets used in transactions submitted by asset-moving featured apps.

With the default values proposed in this CIP, a DvP (Delivery versus Payment) transaction rewards
the traded assets with 10 cents for every $1 of burn spent on the transaction.

#### Fair Treatment of Canton Coin

We expect featured assets to share some of their extra rewards from asset-moving apps with the
apps that moved them.
To ensure that CC will be tradeable on these asset-moving apps as well, the mechanisms implemented
do create extra rewards for the asset-moving apps that trade CC.
With the default values proposed in this CIP, these extra rewards correspond to 5 cents for every $1 of burn spent on the transaction.
...
- 2026-09-21: Propose amendment to introduce featured app categories, rebates for featured app submissions, and incentives to reward featured asset-moving apps using featured assets.
- 2026-06-30: Propose amendment for featured app weights.
