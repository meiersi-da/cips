...
leo@c7.digital  
10:54 PM Jan 12  
Why not lean into Daml and encode this with an appropriate ADT?  
Simon Meier  
Simon Meier  
8:41 AM Jan 13  
Yeah, I'm coming to the same conclusion. I sort of liked the key-value structure, as it is nice to work with in JSON. 

However, it does add a fair bit of complexity; and I'll want to see how everything looks if we just go with (subject, property, value) triples.  
leo@c7.digital  
leo@c7.digital  
3:49 PM Jan 13  
Initially I agreed with you that we do want the uniqueness of the subject, property pair but I can imagine cases were we would want duplicates:

\`("leonid::1220..", "c7.org/alias", "leo")\`  
\`("leonid::1220..", "c7.org/alias", "leonid")\`  
leo@c7.digital  
leo@c7.digital  
3:51 PM Jan 13  
Actually ... just came up with a more real life scenario of needing to support titles; there was a time where

\`("Emnet::1220..", "digitalasset.com/title", "CFO")\`  
\`("Emnet::1220..", "digitalasset.com/title", "COO")\`  
Simon Meier  
Simon Meier  
4:10 PM Jan 13  
Thanks for the good examples. The multi-set semantics that this implies also works has aligns nicely with the fact that multiple credentials can define the same property for the same subject.

Vladislav Kokosh  
11:39 PM Jan 20  
createdAt is currently optional, but the Lookup API text relies on a "record time" to make last-write-wins deterministic. To avoid diverging client/provider behaviour, could we pin this down as a single rule:  
1\) what exactly is the "record time" used for last-write-wins (is it CredentialView.createdAt?)  
2\) is it guaranteed to be present for all records returned by the API (if yes, please make that a MUST for conforming registries/API providers)  
3\) what is the deterministic tie-breaker when two records have the same record time (e.g., order by (recordTime, contractId))? Without this, different Scan backends / clients can compute different "latest" records even from the same dataset.  
Show less  
Simon Meier  
Simon Meier  
4:45 PM Jan 23  
On 1: record-time is assigned as part of the Canton Protocol and refers to the time at which the confirmation request for a Daml transaction was sequenced. It is served on the Ledger API together with every transaction: https://github.com/digital-asset/canton/blob/be5399bd5509cd24fdbb1e7d2ed095c532c6dff1/community/ledger/ledger-json-api/src/test/resources/json-api-docs/openapi.yaml\#L5355

On 2: yes it is guaranteed to be present on the off-ledger credential registry API

On 3: contract-id is the deterministic tie-breaker. It will only be used for credentials created in the very same Daml transaction.

Wayne Collier  
4:12 AM Jan 12  
Does renewal build on top of expiresAt, so I can set up an automatic renewal?  
Simon Meier  
Simon Meier  
10:52 AM Jan 12  
Note that renewal of credentials is best done by creating a new credential 24h ahead of time to avoid contention with existing prepared transactions referencing the old credential.

The CIP currently does not specify who automates renewal. I'd expect the credential issuers to take care of that for the ones they publish.

For self-published credentials, I'd expect wallet providers to support users. Either just at the UI level (clearly highlight when the next renewal is due); or at the workflow level: offer a paid service to maintain a bunch of self-published credentials.

Complementary, we could also have the validator app support registering self-published credentials that should be kept up to date.

I'd suggest we do not specify the method as part of this CIP, but leave it to the market to determine how to best support the users in keeping the credentials current.  
Show less
...
Frank Preiwuss  
12:07 PM Jan 20  
does that mean the registry policy could say: we keep credentials only active for 12 month?  
Simon Meier  
Simon Meier  
1:12 PM Jan 20  
the registry operator defines it. The DSO registry will likely expire credentials by default after 90 days, so that the traffic cost of establishing them covers the cost of storing and serving them.  
Frank Preiwuss  
Frank Preiwuss  
4:42 PM Jan 20  
would it possible to set the expiry to something related when they were fetched for the last time ?  
Simon Meier  
Simon Meier  
4:57 PM Jan 20  
Hmm... are you saying "fetched" on-ledger? That sounds interesting, as there is traffic that's being paid. The problem is though that clients of the registry will nevertheless have to implement renewal in case there was no fetch. Also keeping track of that information on-ledger is non-trivial, unless we make a fetch recreate the contract, which then results in contention.

Can you elaborate what property you are after? 

One thing I did consider is to figure out a way for the credential factory to take an extra payment to also support a higher expiry time. How would that interact with what you have in mind?  
Show less  
Frank Preiwuss  
Frank Preiwuss  
1:54 PM Jan 21  
I’m mainly looking for a way to keep credentials active while they’re being used. I was thinking that “being used” could mean on-ledger access, which would also incur traffic costs. However, I agree that this might overcomplicate the solution, especially since issuers would need to renew credentials anyway or perform regular checks to confirm that a credential is still active.  
Show less  
Frank Preiwuss  
Frank Preiwuss  
2:02 PM Jan 21  
Adding a payment mechanism or deposit to the credential also seems like a good option

Vladislav Kokosh  
12:18 AM Jan 21  
This reads like a security invariant, but it’s currently only a convention enforced by each interface implementation (“provided all vetted packages check expectedAdmin”). Could you clarify the threat model and make the guarantee mechanical where possible? At minimum, please state explicitly that callers MUST rely on package vetting / trusted participants for safety, and that expectedAdmin alone is insufficient if an implementation omits or mis-implements this check. But maybe I've changed my mind here.  
Show less  
Simon Meier  
Simon Meier  
4:47 PM Jan 23  
This is a constraint on implementations that I expect to be checked as part of the security audit required from the registry app provider by a customer of the registry app.

As you call out correctly, the customer deciding to vet the implementation .dar signals to the network that they are OK with the using that particular implementation.  
Show less

Vladislav Kokosh  
11:59 PM Jan 20  
probably alone feels underspecified for pagination: multiple events can share the same record time, so different backends/clients may form different page boundaries and indexers can see gaps/duplicates on retry. Would it be better to define a total ordering and cursor explicitly, e.g. order by (recordTime, stable unique event identifier) and use that pair as the pagination cursor?  
Show less  
Simon Meier  
Simon Meier  
4:48 PM Jan 23  
yes agreed. This will also become more clear when adding the OpenAPI definitions for these endpoints.

Vladislav Kokosh  
11:40 PM Jan 20  
This example mixes the "(property, subject, value)" notation with the earlier key encoding rule (namespace/property\[\!subject\]). Would it be clearer to show the concrete Claims.values key here, e.g. cip-\<nr\>/profile.website (no \!subject since subject defaults to holder), instead of property="profile.website", subject="\<p\>"?  
Vladislav Kokosh  
Vladislav Kokosh  
11:43 PM Jan 20  
and more, would it be clearer to use the "holder" as the subject here? Earlier you define that if there is no \!subject suffix then the subject is implicitly the credential’s holder, and you recommend canonical form without repeating \!holder. For this self-published profile example, using an implicit holder-subject (rather than subject="\<p\>") would keep the example aligned with the encoding model  
Show less  
Simon Meier  
Simon Meier  
4:51 PM Jan 23  
Agreed that this can be made more clear. Let me do so in my next polishing pass.

I do plan to uniformly switch to triples.

Edward Newman  
8:18 PM Jan 9  
This one really intrigues me and has been discussed since we all were in 162 5th Ave. Wayne and I discussed at length today.

My theory is that due to regulatory and legal reasons, each entity performed KYC themselves on their customers. So FinSvcs entity A will create a "KYC Verification" credential about entity B and for A's use. 

I would be interested whether entity C would use (or be allowed to use due to regulatory/financial controls) to use a KYC credential issued by A for B. Specifically interested in how people have resolved issues like legal liability on A if they fail to do the KYC process sufficiently, i.e. B commits fraud against C who relied on KYC by A.

Do we have equivalent examples of such outsource KYC processes?  
Show less  
Simon Meier  
Simon Meier  
8:52 AM Jan 12  
The one I know of is the ISR using ID.me to do ID verification of the users of their online services: https://www.irs.gov/newsroom/new-identity-verification-process-to-access-certain-irs-online-tools-and-services

As you can see from the text, I think what should be in place are the tools for such out-sourcing to happen; and for that out-sourcing to become standardized over time. Whether that actually happens sort is then a question that we'll learn the answer to over time.  
Show less  
Simon Meier  
Simon Meier  
8:55 AM Jan 12  
One other case where I'd expect cross-entity use of KYC verification is when a single org is organized as multiple holdings that have different party-ids on-ledger; e.g., a retail bank where the branches reuse credentials issued by a common KYC verification service (e.g., for accredited investor verification).  
Comments above copied from original document  
Edward Newman  
Edward Newman  
3:19 PM Jan 12  
IMHO IRS is just the IRS doing it for themselves but using an outsource provider for tech and process. The second case also seems to be an entity KYC'ing itself.  
Simon Meier  
Simon Meier  
8:52 AM Jan 13  
I agree with that perspective from a legal sense. However, at the technical level, I wouldn't be surprised if the issuer party for these KYC services is a distinct party, and the issuance app itself is separate from the app using the credentials issued by that app.
